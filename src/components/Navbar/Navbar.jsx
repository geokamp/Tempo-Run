        ...
        <div className="justify-between mx-auto lg:w-full md:items-center md:flex">
          ...
          <div>
            <div className="flex items-center justify-between py-1 md:py-1 md:block">
              {/* Logo section */}
              <Link className="text-3xl text-red-700 font-semibold tracking-[0.1rem] flex items-end gap-x-1 relative">
                Tempo Run
              </Link>
            ...
            <ul className="list-none lg:flex md:flex sm:block block items-center gap-x-5 gap-y-16">
              {navItems.map((item, index) => (
                <li key={index}>
                  <Link
                    to={item.link}
                    className="text-gray-500 text-lg font-medium hover:text-red-700 ease-out duration-700"
                  >
                    {item.name}
                  </Link>
                </li>
